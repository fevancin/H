...
def common_main_checker(command_name, function_to_call, needs_results):

    parser = argparse.ArgumentParser(prog=command_name, description='Check data validity')
    ...
    if needs_results:
        ...
        if not results_directory_path.exists():
            ...
            raise ValueError(f'Results \'{results_directory_path}\' is not a directory')

    if is_verbose:
        print(f'Checking data in directory \'{group_directory_path}\'')
    ...
    for instance_file_path in input_directory_path.iterdir():
        ...
        if needs_results:
            results_file_name = f'{instance_file_name}_results'
            results_file_path = results_directory_path.joinpath(f'{results_file_name}.json')
        ...
        if is_verbose:
            print(f'Checking instance \'{instance_file_name}\'... ', end='')
            instance_number += 1

        try:
            if needs_results:
                function_to_call(instance, results)
            else:
                function_to_call(instance)
            print('All good! :)')
    ...
    if is_verbose:
        ...
        print(f'End checking process. Time elapsed: {end_time - start_time} seconds')
        print(f'Checked {instance_number} instances')